...
class UserService:
    ...
    async def get_current_user(token: str):
        try:
            ...
            try:
                user = UserService.get_user_by_id(conn, user_id)
            finally:
                conn.close()
    ...
    async def get_subscription(jwt_token: str):
        ...
        try:
            ...
            conn = UserService.get_db_connection()
            try:
                user_info = UserService.get_user_by_id(conn, user_id)
            finally:
                conn.close()
    ...
    def _handle_telegram_auth(auth_data: str):
        ...
        data = UserService.parse_telegram_data(auth_data)
    ...
    def _get_user_subscription_info(conn, telegram_id: int):
        user_info = UserService.get_user_by_id(conn, telegram_id)
        logger.info(f"User info retrieved: {user_info}")
        if not user_info:
            logger.info(f"Subscription inactive for user {telegram_id}")
            return SubscriptionCheckResponse(status="inactive", display_text="Подписка неактивна", config_url=None)
        return user_info
    ...
    def _handle_active_subscription(conn, user_info: User, telegram_id: int):
        try:
            ...
            if expiry_datetime < current_datetime:
                UserService.update_user_status(conn, telegram_id, "expired")
            ...
            timeleft = UserService.format_time_left(user_info.expiry)
            jwt_token = user_info.jwt_token
    ...
    @staticmethod
    def _handle_inactive_subscription(telegram_id: int):
        logger.info(f"User {telegram_id} is inactive (created but no subscription)")
        return SubscriptionCheckResponse(
            status="inactive",
            display_text="Подписка неактивна",
            config_url=None
        )
    ...
    async def check_subscription(auth_data: str) -> SubscriptionCheckResponse:
        ...
        conn = None
        try:
            telegram_id = UserService._handle_telegram_auth(auth_data)
            logger.info(f"Checking subscription for user with telegram_id: {telegram_id}")

            conn = UserService.get_db_connection()
            logger.info("Database connection successful")

            user_info = UserService._get_user_subscription_info(conn, telegram_id)

            if isinstance(user_info, SubscriptionCheckResponse):
                return user_info
            if user_info.status == "active":
                return UserService._handle_active_subscription(conn, user_info, telegram_id)
            elif user_info.status == "expired":
                return UserService._handle_expired_subscription(telegram_id)
            elif user_info.status == "inactive":
                return UserService._handle_inactive_subscription(telegram_id)
            else:
                return UserService._handle_unknown_subscription_status(telegram_id, user_info.status)
